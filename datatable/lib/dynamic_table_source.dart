...
class DynamicTableSource extends DataTableSource {
  ...
  final Future<bool> Function(int index, List<dynamic> row)? onRowDelete;
  ...
  final MyViewModel _viewModel = locator<MyViewModel>();
  ...
  void sort<T>(
      Comparable<T> Function(DynamicTableDataRow d) getField, bool ascending) {
    data.sort((a, b) {
  ...
  void addRow() async {
    final data = await _viewModel.showFullScreenDialog();
    // print(data);
    ...
    if (data == null) {
      ...
      // updateRow(0, data);
      // print(data);
      String datetime_str = data[2];
      DateTime  datetime_obj = DateTime.parse(datetime_str);

      // data[2] = datetime_obj;
      var newData=[data[0],data[1],datetime_obj,data[3],data[4]];

      print(newData);
      insertRow(0,newData, isEditing: false);
      deleteRow(-1);
      //  handleRowAddition(data);
    }
    if (!showActions) {
      throw Exception(
  ...
  List<DataCell> _addActionsInCell(
    ...
    if (showDeleteAction) {
      actions.add(DynamicTableActionDelete(
        ...
        showAlways: true,
        onPressed: () async {
          var response = await onRowDelete?.call(
              row,
              cells.map((e) {
                return e.value;
              }).toList());
